  ...
  const onSubmit = async (event) => {
    try {
      const response = await axios.get(
        `${baseUrl}/auth/doctors?email=${event.email}`
      );
  ...
  const handleUpdateInfo = async (e) => {
    try {
      const response = await axios.put(
        `${baseUrl}/auth/updateInfo/${searchData[0]._id}`,
        userData
      );
      handleClose();
    } catch (err) {
      ...
      <div className="col-md-10 p-5 pr-4 m-0 patient-container">
        ...
        <table className="table shadow-lg p-5 mt-4">
          <thead className="thead-dark">
            <tr>
              <th className="text-primar text-center" scope="col">
                Sr No
              </th>
            ...
            <form>
              <div className="mb-3">
                ...
                <input
                  ...
                  id="doctorUserName"
                  name="username"
                  defaultValue={searchData[0]?.username}
                />
              </div>
              <div className="mb-3">
                ...
                <input
                  ...
                  id="doctorEmail"
                  name="email"
                  defaultValue={searchData[0]?.email}
                />
              </div>
              <div className="mb-3">
                <select
                ...
                >
                  <option
                    value={
                      searchData[0]?.isAdmin
                        ? "2"
                        : searchData[0]?.isDoctor
                        ? "1"
                        : "0"
                    }
                    defaultValue
